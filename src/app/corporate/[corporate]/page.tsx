...
const Page: React.FC = () => {
  const { corporate } = useParams(); // Accessing the dynamic 'city' parameter
    ...
    <div>
      ...
      <div>
        <h1 className="flex justify-center mt-4 text-2xl font-bold text-center text-blue-400 md:text-4xl">
          Corporate Cabs Service in {corporate}
        </h1>
        <h2 className="mt-4 ml-4 text-xl font-semibold text-gray-800">
          Looking for a Cab in {corporate}?
        </h2>
        <h6 className="max-w-3xl mx-auto mt-2 ml-5 text-base leading-6 text-gray-600">
          {corporate} Cab Booking at Affordable fare. We at WTL provide quality services for cab bookings for Outstation, 
          One-Way, Corporate Cabs, and Round Trips from Pune, Maharashtra, and other cities. Book your next cab with WTL 
          and get a discount on the total fare to {corporate}.
        </h6>
      </div>
      <div>
        <CabImg />
      ...
      <div>
        <h1 className="flex justify-center mt-6 text-xl">Corporate Cabs Service in {corporate}</h1>
        <h6 className="flex justify-center mt-2 text-2xl font-bold text-center text-black-300 md:text-4xl">
          About WTL Pvt Ltd
        </h6>
        <h1 className="ml-4">
          Welcome To "WTL" Cab Booking Service In {corporate}! WTL is an established car rental service provider in India, 
